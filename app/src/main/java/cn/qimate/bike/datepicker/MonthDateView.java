package cn.qimate.bike.datepicker;
...
public class MonthDateView extends View {
	...
	private int mCircleColor = Color.parseColor("#20c48a");
	...
	public MonthDateView(Context context, AttributeSet attrs) {
		...
		// 设置圆圈半径为屏幕宽度0.06倍
		mCircleRadius = (int) (metric.widthPixels * 0.05);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		String year_month = mSelYear + String.format("%2d", (mSelMonth + 1)).replace(" ", "0");
		for (int day = 0; day < mMonthDays; day++) {
			...
			if (daysHasThingList != null && daysHasThingList.size() > 0) {
				if (daysHasThingList.contains(yearmonthday)) {
					mPaint.setColor(mSelectDayColor);
				} else {
					mPaint.setColor(mDayColor);
				}
