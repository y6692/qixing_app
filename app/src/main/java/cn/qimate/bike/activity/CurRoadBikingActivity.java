...
//                            customDialog11.show();
                    ...
                    case Config.CLOSE_ACTION:
                        Log.e("biking===", "CLOSE_ACTION==="+isStop);
                        if (loadingDialog != null && loadingDialog.isShowing()){
                            loadingDialog.dismiss();
                        }
                        if (lockLoading != null && lockLoading.isShowing()){
...
//                        tvCz.setText("当前操作：搜索信标成功"+data.substring(2*10, 2*10+2)+"==="+data.substring(2*11, 2*11+2)+"==="+data);
                        ...
                        Log.e("XINBIAO_ACTION===", "当前操作：搜索信标成功"+data.substring(2*10, 2*10+2)+"==="+data.substring(2*11, 2*11+2)+"==="+data);

                        if("000000000000".equals(data.substring(2*4, 2*10))){
                            major = 0;
                        }else{
                            major = 1;
                        }
...
//                getBleRecord();
                ...
                Log.e("biking_endBtn===", type+"==="+major+"==="+isContainsList.contains(true));
...
//                        UIHelper.goToAct(context, ServiceCenter0Activity.class);
                        ...
                        Intent intent = new Intent(context, ServiceCenter0Activity.class);
                        intent.putExtra("bikeCode",bikeCode);
                        startActivity(intent);
//                        scrollToFinishActivity();
...
//    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.e("biking===00", "188===="+resultCode);

        if(resultCode == RESULT_OK){
            Log.e("biking===0", "188===="+requestCode);
