...
//                        }
    ...
    private void change_phone(String telcode) {
        Log.e("change_phone===0", telphone+"==="+telcode);
        ...
        HttpHelper.post(context, Urls.change_phone, params, new TextHttpResponseHandler() { //TODO 验证码错误
            ...
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                onFailureCommon("ncpa===change_phone", throwable.toString());
            }
            ...
            public void onSuccess(int statusCode, Header[] headers, final String responseString) {
                m_myHandler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            Log.e("change_phone===", "==="+responseString);

                            ResultConsel result = JSON.parseObject(responseString, ResultConsel.class);

                            Log.e("change_phone===1", result.getData()+"==="+result.getStatus_code());

                            if (result.getStatus_code()==200) {
                                Toast.makeText(context,"换绑成功",Toast.LENGTH_SHORT).show();
